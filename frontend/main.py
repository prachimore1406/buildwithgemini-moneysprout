...
RESOURCE = os.environ.get(
    ...
    "projects/391165179520/locations/us-east1/reasoningEngines/1677480910032732160",
)
...
async def get_dashboard():
    """Returns live Firestore DB collection data for dashboard tiles."""
    try:
        db = get_firestore_client()
        goals = [doc.to_dict() for doc in db.collection("savings_goals").stream()]
        streaks = [doc.to_dict() for doc in db.collection("discipline_and_etiquette_streaks").stream()]
        badges = [doc.to_dict() for doc in db.collection("badges_and_achievements").stream()]
        waiting_room = [doc.to_dict() for doc in db.collection("delayed_gratification_waiting_room").stream()]
        chores = [doc.to_dict() for doc in db.collection("chores_and_allowance").stream()]
        return {
            "goals": goals,
            "streaks": streaks,
            "badges": badges,
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    so the UI renders the card cleanly without raw JSON text leaking into chat.
    """
    import json
    import re
